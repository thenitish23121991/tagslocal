...
var db = require('monk')('tagslocal_me:stayfoolish@ds049548.mongolab.com:49548/heroku_app24357007');
...
module.exports = {
	crawl_site_phonearena: function(url,brand){
		request(url,function(error,response,html){
		
		if(!error && response.statusCode == 200){
			
		var $ = cheerio.load(html);
		var products = $('#phones .s_block_4');

		
		products.each(function(i,element){ 
				global.specs_row_arr = {};
				var href = products.eq(i).find('a').eq(0).attr('href');
				 request("http://www.phonearena.com"+href,function(error,response,html){
				 	var $ = cheerio.load(html);

				 	var title = $('#phone h1').eq(0).text();
				 	title = title.replace("Follow","");
				 	title = title.replace("Following","");
				 	title = title.replace(/\n/,"");
				 	title = title.trim();
				 	var desc = $('#phone .desc').text();
				 	var imgsrc = $('#phone .lead img').attr('src');
				 	var specs_html = $('#phone_specificatons').html();
				 	var rows = $('#phone_specificatons li.s_lv_1');
					
				 	rows.each(function(i,element){

							var row_name = rows.eq(i).find('strong.s_lv_1').text();
				
							var value = rows.eq(i).find('ul.s_lv_1 li').text();
							
							if(row_name == "OS:" || row_name == "Device Type:" || row_name == "Dimensions:" || row_name == "Resolution:" || row_name == "Camera:" || row_name == "Processor:" || row_name == "Browser:" || row_name == "Navigation:" || row_name == "Wi-Fi:" || row_name == "Weight:" || row_name == "Physical size:" || row_name == "Pixel Density:" || row_name == "Touchscreen:" || row_name == "Capacity:" || row_name == "Speakers:" || row_name == "USB:" || row_name == "Positioning:" || row_name == "Graphics processor:" || row_name == "System memory:"){
				 ...
				 db1.add_product_phonearena(title,desc,imgsrc,"phones",brand,"",global.specs_row_arr);
				 });
...
request(url, function(error,response,html){
			if(!error && response.statusCode == 200){
				...
				products.each(function(i,element){
					...
					title = title.replace("Cellular","");
					title = title.replace("3G","");
					title = title.replace("+","");
					title = title.trim();
