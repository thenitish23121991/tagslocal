...
var prev_add = products.find({name:name});	
prev_add.on('complete',function(err,docs){
if(docs.length == 0){	
var promise = products.insert({'name':name,'description':description,'pic_path':pic_path,'category':category,'brand':brand,'price':price,'specification_html':specs_html,'url':name,'views':0});
...
var promise = products.insert({'name':name,'url':name,'description':description,'pic_path':pic_path,'category':category,'brand':brand,'price':price,'specification_array':JSON.stringify(specs_arr)});
promise.on('success',function(){
console.log(name);
});
promise.on('error', function(err){
